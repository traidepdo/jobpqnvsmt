...
import AppliedJobsPage from '../candidate/applications/page';
...
function JobsPageContent() {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        ...
        const savedRes = await fetch('/api/candidate/saved-jobs');
        if (savedRes.ok) {
          const d = await savedRes.json();
          const savedIds = new Set<string>((d.savedJobs || []).map((s: any) => s.job?.id).filter(Boolean));
          setSavedJobs(savedIds);
        }
        const appliedRes = await fetch('/api/jobs/applied');
        if (appliedRes.ok) {
          const d = await appliedRes.json();
          const appliedIds = new Set<string>((d || []).map((j: any) => j.id).filter(Boolean));
  ...
  const checkAppliedJobs = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setSavedJobs(new Set(data.map((job: JobItem) => job.id)));
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const toggleSave = async (id: string) => {
    ...
    try {
      ...
      if (saved) {
        const res = await fetch(`/api/candidate/saved-jobs?jobId=${id}`, { method: 'DELETE' });
