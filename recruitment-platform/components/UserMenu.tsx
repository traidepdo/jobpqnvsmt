...
export default function UserMenu({ user }: UserMenuProps) {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {/* Menu thả xuống */}
            {isOpen && (
                <div className="absolute right-0 mt-2.5 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 py-2.5 z-50 animate-in fade-in slide-in-from-top-3 duration-200">
                    {/* Header thông tin người dùng */}
                    <Link
                        href={config.dashboardUrl}
                        onClick={() => setIsOpen(false)}
                        className="px-4 py-3 border-b border-gray-50 flex flex-col gap-1.5 hover:bg-gray-50/80 transition-colors cursor-pointer group"
                    >
                    ...
                    {/* Danh sách chức năng */}
                    <div className="px-1.5 py-1.5 border-b border-gray-50 flex flex-col gap-0.5">
                        <Link
                            href={config.dashboardUrl}
                            onClick={() => setIsOpen(false)}
