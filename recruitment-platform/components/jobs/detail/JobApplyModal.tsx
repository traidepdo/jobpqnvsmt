...
export default function JobApplyModal({
  ...
  useEffect(() => {
    if (userResumes && userResumes.length > 0) {
      const defaultResume = userResumes.find((r: any) => r.isDefault === true || r.isDefault === 'True');
      if (defaultResume) {
  ...
  const cvisdefault = userResumes.find((r: any) => r.isDefault === 'True' || r.isDefault === "TRUE" || r.isDefault === true || r.isDefault === "true");
  const cvnotdefault = userResumes.filter((r: any) => !r.isDefault || r.isDefault === 'false' || r.isDefault === 'False' || r.isDefault === "FALSE" || r.isDefault === false || r.isDefault === "false");
  const listcv = cvisdefault ? [cvisdefault, ...cvnotdefault] : cvnotdefault;
          ...
          <form onSubmit={handleApplySubmit} className="p-6 flex flex-col gap-4">
            ...
            <div>
              ...
              {listcv?.length > 0 ? (
                ...
                >
                  {listcv[0].isDefault ? (
                    <option value={listcv[0].id}>{listcv[0].title} (mặc định)</option >) : (
                    <option value={listcv[0].id}>{listcv[0].title}</option>
                  )}
                  {listcv
                    .slice(1)
                    .map((r: any) => (
